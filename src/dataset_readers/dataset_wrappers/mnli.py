...
label2text = get_mapping_token("mnli")


@field_getter.add("X")
def get_X(entry):
    return entry['premise'] if 'X' not in entry.keys() else entry['X']
...
def get_C(entry):
    return entry['hypothesis'] if 'C' not in entry.keys() else entry['C']
...
def get_ALL(entry):
    return f"{entry['hypothesis']}\t {entry['premise']}" if 'ALL' not in entry.keys() else entry['ALL']


class MNLIDatasetWrapper(DatasetWrapper):
    name = "mnli"
    ...
    def __init__(self, dataset_path=None, dataset_split=None, ds_size=None):
        ...
        if dataset_path is None:
            self.dataset = load_dataset("LysandreJik/glue-mnli-train",split=dataset_split)
